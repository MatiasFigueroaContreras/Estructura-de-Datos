...
arista crear_arista(int v, int w, int flujo){
    //Crea una arista con los valores entregados en la entrada.
    arista newArista;
    newArista.v = v;
    newArista.w = w;
    newArista.flujo = flujo;
    return newArista;
}

enlace crear_nodo(int valor, enlace sig, int flujo){
...
void insertar_arista(Grafo grafo, arista aris){
    ...
    if(aris.v < grafo->numV && aris.w < grafo->numV){
        ...
        grafo->adj[aris.v] = crear_nodo(aris.w, grafo->adj[aris.v], aris.flujo);
        grafo->numA++;
    }
}

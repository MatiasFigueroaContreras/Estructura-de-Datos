...
enlace crear_nodo(int valor, enlace sig){
    ...
    newNodo->tipo = 0; // tipo y marca 0 para poder trabajar el problema
    newNodo->marca = 0; 
...
Grafo inicia_grafo_completo(int numV){
    ...
    for(i = 0; i < numV; i++){
        ...
        for(j = 0; j < numV; j++){
            if(j != i){
                newGrafo->adj[i] = crear_nodo(j, newGrafo->adj[i]);
            }
        }
    }
    return newGrafo;
...
void insertar_arista(Grafo grafo, arista aris){
    ...
    if(aris.v < grafo->numV && aris.w < grafo->numV){
        ...
        if(aris.v == aris.w){
            grafo->adj[aris.v] = crear_nodo(aris.w, grafo->adj[aris.v]); //(modifacion) Verificacion que nos ayuda a resolver nuestro problema
        }
        else{
            grafo->adj[aris.v] = crear_nodo(aris.w, grafo->adj[aris.v]);
            grafo->adj[aris.w] = crear_nodo(aris.v, grafo->adj[aris.w]);
        }
        grafo->numA++;
    }
}
